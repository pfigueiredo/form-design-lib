...
/**
 * Array Binding Strategy
 * 
 * Handles paths with array indices like:
 * - "userExperience[0].employer"
 * - "items[1].name"
 * - "array[0][1].property" (nested arrays)
 */
export class ArrayBindingStrategy implements BindingStrategy {
  canHandle(path: string): boolean {
    // Handle paths that contain array indices (brackets with numbers)
    return /\[\d+\]/.test(path)
  }
  ...
  resolve(path: string, store: Record<string, any>): any {
    ...
    // Use getNestedValue which handles array indices
  ...
  set(path: string, value: any, store: Record<string, any>): Record<string, any> {
    ...
    // Set nested value using relative path (setNestedValue handles array indices)
